...
router.put('/profile', requireAuth, async (req, res) => {
    ...
    try {
        const result = await pool.query(`INSERT INTO user_profiles (user_id, gender, date_of_birth)
       VALUES ($1, $2, $3)
       ON CONFLICT (user_id) DO UPDATE SET
         gender = EXCLUDED.gender,
         date_of_birth = EXCLUDED.date_of_birth,
         updated_at = NOW()
       RETURNING *`, [req.userId, gender, date_of_birth]);
...
router.put('/lifestyle', requireAuth, async (req, res) => {
    ...
    try {
        const result = await pool.query(`INSERT INTO lifestyle_data (user_id, cigarettes_per_day)
       VALUES ($1, $2)
       ON CONFLICT (user_id) DO UPDATE SET
         cigarettes_per_day = EXCLUDED.cigarettes_per_day,
         updated_at = NOW()
       RETURNING *`, [req.userId, cigarettes_per_day]);
...
router.put('/medical-history', requireAuth, async (req, res) => {
    ...
    try {
        const result = await pool.query(`INSERT INTO medical_history (user_id, conditions, has_test_results)
       VALUES ($1, $2, $3)
       ON CONFLICT (user_id) DO UPDATE SET
         conditions = EXCLUDED.conditions,
         has_test_results = EXCLUDED.has_test_results,
         updated_at = NOW()
       RETURNING *`, [req.userId, conditions, has_test_results ?? false]);
